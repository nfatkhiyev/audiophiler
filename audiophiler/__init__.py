...
def upload(auth_dict=None):
    ...
    for f in uploaded_files:
        ...
        #Get file_id
        file_id = File.query.order_by(File.file_id.desc()).first().file_id

        #Add the conversion to the queue
        q.enqueue(process_audio_task, get_file_s3(s3_bucket, file_hash), file_id, filename, author)
...
def get_harold_list(uid):
    ...
    harolds = [harold.file_id for harold in harold_list]
